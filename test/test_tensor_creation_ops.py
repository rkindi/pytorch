...
class TestTensorCreation(TestCase):
    ...
    def test_meshgrid_vs_numpy(self, device):
        # Possible shapes to the random tensors. Note that [] produces
        # a 0D tensor.
        shapes = [[], [1], [2], [3], [5]]

        # Generate test cases that pass 1, 2, or 3 tensors to
        # meshgrid. The function doesn't make much sense beyond three
        # dimensions and is not supported with zero tensors.
        cases = []
        for num_tensors in range(1, 4):
            cases.extend(product(shapes, repeat=num_tensors))
